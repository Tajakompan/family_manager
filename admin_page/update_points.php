...
function respond_update_points(bool $ok, ?string $error = null, array $payload = [], int $status = 200): void
{
...
if ($user_id <= 0) {
    respond_update_points(false, "missing_user", [], 422);
}

if ($points_raw === "" || filter_var($points_raw, FILTER_VALIDATE_INT) === false || (int)$points_raw < 0) {
    respond_update_points(false, "invalid_points", [], 422);
}
...
if (!$user_exists) {
    respond_update_points(false, "missing_user", [], 404);
}

$sql = "UPDATE app_user SET user_points = ? WHERE id = ? AND family_id = ?";
...
if (!$execute_ok) {
    respond_update_points(false, "save_failed", [], 500);
}

respond_update_points(true, null, [
    "user" => [
        "id" => $user_id,
        "user_points" => $points,
    ],
