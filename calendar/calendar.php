...
$year_now  = isset($_GET['year'])  ? (int)$_GET['year']  : date('Y');
$month_now = isset($_GET['month']) ? (int)$_GET['month'] : date('n');
$prev_year = $year_now;
$next_year = $year_now;
...
$start_day = date('N', strtotime("$year_now-$month_now-01")); //N- numeric representation for a day 1=pon, 7=ned; prvi datum v mesecu
...
for ($d = 1; $d <= $days_in_month; $d++) {
    ...
    foreach ($dayEvents as $e) {
        $events_by_date[$dateKey][] = [
            "id" => $e["id"],
            "name" => $e["name"],
            "event_time" => ($e["whole_day"] == 1) ? "celodnevni dogodek" : ($e["event_time"] ?? ""),
            "event_time_raw" => $e["event_time"] ?? "",
        "whole_day" => (int)$e["whole_day"],
            "location" => ($e["location"] ?? "") === "" ? "/" : $e["location"],
            "description" => ($e["description"] ?? "") === "" ? "/" : $e["description"],
            "user_id" => $e["created_by_app_user_id"],
            "reminder" => $e["reminder"] ?? ""
        ];
    }
}
...
<body>
    ...
    <div class="sredina">
        <div class="mesec">
            <a class="nav-btn" href="?month=<?= $prev_month ?>&year=<?= $prev_year ?>">nazaj</a>
            <span class="ime"><?= $months[$month_now -1] . " " . $year_now?></span>
            <a class="nav-btn" href="?month=<?= $next_month ?>&year=<?= $next_year ?>">naprej</a>
        </div>
        <table id="outer_table">
            <tr><th>PON</th><th>TOR</th><th>SRE</th><th>ČET</th><th>PET</th><th>SOB</th><th>NED</th></tr>
            <tr>
            <?php
            $day = 1;
            ...
            while($day <= $days_in_month){
                if($day != 1 && ($day + $start_day - 2)%7 == 0)
                    echo "</tr><tr>";
                $class = ($day == date('j') && $month_now == date('n')) ? 'danes' : '';
    ...
    <!-- desno brez gumba - obrazec za dodajanje dogodka -->
    <div class="desno" id="add_event_form">
        ...
        <form method="post" data-mode="add" action="add_event.php?month=<?= $month_now ?>&year=<?= $year_now ?>">
            ...
            <div class="input_row"><label>Naziv: </label><input type="text" name="name"></div>
            <div id="error_date" class="hidden_error"></div>
            <div class="input_row"><label>Datum: </label> <input type="date" name="date" id="date_input"></div>
            <div id="error_time" class="hidden_error"></div>
            <div class="input_row"><label>Ura: </label> <input type="time" name="time"></div>
            <div class="input_row checkbox"><input type="checkbox" name="whole_day" id="whole_day"><label for="whole_day">Celodnevni dogodek</label></div>
            <div class="input_row"><label>Lokacija: </label><input type="text" name="location"></div>
            <div class="input_row"><label>Opis: </label> <input type="text" name="description"></div>
            <div class="input_row"><label>Opomnik: </label><input type="datetime-local" name="reminder"></div>
            <div class="input_row checkbox"><input type="checkbox" name="just_for_creator" id="just_cre"><label for="just_cre">Dogodek vidim le jaz</label></div>
